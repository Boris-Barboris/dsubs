...
Modified by Boris-Barboris to support efficient removal with predicate.
...
/**
Removes all elements in the list that match the predicate $(D pred).
*/

    void removePred(bool delegate(T) pred)
    {
...
applies function on each removed element.
*/
    ...
    void removePredFirst(bool delegate(T) pred, void delegate(T) func)
    {
        assert(_root !is null, "Cannot remove from an un-initialized List");
        auto p = _first;
        while (p !is _root)
