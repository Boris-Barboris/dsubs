...
mkdir -p ~/dlang && wget https://dlang.org/install.sh -O ~/dlang/install.sh && \
    bash ~/dlang/install.sh install dmd-2.090.0
...
echo "dsubs_libs contain sfml fork and openal-soft, compiled by gcc/g++ 7.1.0 against glibc 2.23.
Requires your distro analog of libx11-dev and xclip.
Run by invoking run_client.sh" > readme_linux.txt
tar -zcvf dsubs-linux-xenial-amd64.tar.gz dsubs_libs fonts dsubs_client alsoft.ini run_client.sh readme.txt